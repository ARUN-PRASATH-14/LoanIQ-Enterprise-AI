...
with tab1:
    if analyze_btn:
        ...
        with st.spinner("Analyzing Risk Profile..."):
            try:
                pred_resp = requests.post(f"{API_BASE_URL}/predict", json=payload).json()
                shap_resp = requests.post(f"{API_BASE_URL}/shap", json=payload).json()
                rag_resp = requests.post(f"{API_BASE_URL}/rag", json=payload).json()
                fraud_resp = requests.post(f"{API_BASE_URL}/fraud", json=payload).json()
                rec_resp = requests.post(f"{API_BASE_URL}/recommendation", json=payload).json()
                ...
                st.error("Failed to connect to FastAPI Backend. Make sure it is running on http://localhost:8000")
    else:
    ...
    if user_query:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching FAISS Policy Vector DB..."):
                try:
                    chat_resp = requests.post(f"{API_BASE_URL}/chat", json={"query": user_query}).json()
